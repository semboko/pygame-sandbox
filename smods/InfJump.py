...
class InfJump(BaseMod):
    ...
    def handle_pressed_keys(self, keys: Sequence[bool]):
        if not self.locked:
            rc = self.scene.space.segment_query(
                self.scene.player.body.position,
                (self.scene.player.body.position.x, self.scene.player.body.position.y - 100),
                0,
                self.scene.player.sf,
            )
