...
class SaveVideo(BaseMod):
    ...
    def remover(self, n=None):
        if n is None:
            n = self.nam
        self.get_mod("Animate-for-sandbox").remove_img(f"user_data/Save_video/img{n}.jpg")
    ...
    def quit(self, error: str = None):
        if self.on:
            ...
            self.get_mod("Animate-for-sandbox").Video(
                f"user_data/Save_video/save{type(self.scene).__name__}({self.nam}).mp4",
                imgs,
                typew=cv2.VideoWriter_fourcc(*"mp4v"),
            )
